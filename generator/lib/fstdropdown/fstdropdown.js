...
function setFstDropdown() {
    ...
    function createDropdown(select) {
        ...
        select.fstdropdown = {
            ...
            setValue: function(value) { setValue(dropdown, value) },
            setTitle: function(value) { setTitle(dropdown, value) },
            next: function() { nextOption(dropdown) },
            previous: function() { previousOption(dropdown) }
        };
    ...
    function nextOption(dd) {
        var current = dd.querySelector(".fstlist .selected");
        var next = current.nextElementSibling;
        if (next)
            setValue(dd, next.dataset["value"]);
    }

    function previousOption(dd) {
        var current = dd.querySelector(".fstlist .selected");
        var prev = current.previousElementSibling;
        if (prev)
            setValue(dd, prev.dataset["value"]);
    }

    function setTitle(dd, value) {
        dd.querySelector(".fstselected").textContent = value;
        dd.querySelector(".fstlist div.selected").textContent = value;
    }
    ...
    function openSelect(event, open, force) {
        ...
        if (select.dataset["searchdisable"] == null && select.dataset["searchdisable"] != "true") {
            el.querySelector(".fstlist .selected").scrollIntoView({ block: "nearest", inline: "nearest" });;
