...
var card_elemement_generators_expanation = {
    ...
    property: 'Appears as a bold text followed by a second parameter (with a white space in-between).' + preCode('property'),
    ...
    description: 'Appears as a italic bold text followed by a second parameter (with a period in-between).' +
        preCode('description'),
    ...
    fill: 'Fills a space with nothingness, usefull to create footers or headers without anyhting near it.' +
        preCode('fill'),
    section: 'Appears as monospace text with a line beneath it colored the same as the selected color.' + preCode('section'),
    disabled: "Appears as empty text. Useful for when creating notes on a card that won't appear." + preCode('disabled'),
    picture: "<div style='color:red'>Disabled until further notice.</div>",
    icon: 'Appears as a smaller version of a selected icon. It uses the same method that the card back icon uses and is colored like the card color.<br/>' +
...
function card_generate_back(data, options) {
    ...
    var background_text = data.background_text_toggle;
    if (background_text) {
